---
title: "ThyCAT: Through-Year Computerized Adaptive Testing"
output:
  html_document:
...
link-citations: true
vignette: >
  %\VignetteIndexEntry{ThyCAT package}
...
The **ThyCAT** package performs adaptive testing based on the Through-Year DMST design. The Through-Year DMST design is a longitudinal extension of the Dynamic Multi-Stage Test (DMST) design. The DMST design presents each module as an adaptive test assembled dynamically or on the fly, based on test blueprint constraints and interim ability estimates in real time. In addition, the Through-Year extension also provides adaptivity across multiple administrations of an assessment throughout the year.

The Through-Year DMST design in **ThyCAT** package uses an assessment with three administrations (Fall, Winter, and Spring) with two phases within each administration (Phase 1 and Phase 2), so that a student takes six modules in total.
...
The ThyCAT package uses hard constraints to perform overlap control. Three options are available:
...
The **ThyCAT** package supports MLE and MLEF in the `TestDesign` package for $\theta$ estimation. The estimation method must be specified in `createShadowTestConfig()`.
...
In a `thyCAT()` simulation, two types of ability estimates are obtained after completing each module.
...
* Using the routing theta and the cut scores, the achievement level is determined. The achievement level is either:
  ...
  * Proficient-developing
    * In **ThyCAT** package, no distinction is made between Proficient and Developing.
  * Beginning
...
This section explains how to use **ThyCAT** package.
...
module_list <- loadModules(
  ...
  base_path = system.file(package = "ThyCAT")
)
```
...
The final step is to run the main simulation using `thyCAT()`.

```{r, cache = TRUE, results = "hide"}
examinee_list_output <- thyCAT(
  examinee_list          = examinee_list,
  assessment_structure   = assessment_structure,
