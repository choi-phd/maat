...
The **maat** package supports EAP, MLE and MLEF in the `TestDesign` package for $\theta$ estimation. The estimation method must be specified in `createShadowTestConfig()`.
...
### Route chart for the `CI` transition policy condition
...
### Route chart for the `pool_difficulty_percentile` transition policy condition
...
* Plot the true $\theta$ against the estimated $\theta$ across fall, spring, and winter test administrations.
* The argumnents of the `plot()` functions: 
  * `x` is the output object produced by `maat()`.
  * `theta_range` is the $\theta$ ranged used for the scatterplot.
  * `main` is the vector of titles for the plots

### Scatterplot for the `CI` transition policy condition
...
### Scatterplot for the `pool_difficulty_percentile` transition policy condition
