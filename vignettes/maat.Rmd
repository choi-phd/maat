...
The **maat** package performs adaptive testing based on the assessment framework involving multiple tests administered throughout the year using multiple item pools vertically scaled and multiple phases within each administration allowing for transitioning from one item pool (and associated constraints) to another as determined necessary according to a prespecified transition policy to enhance the quality of measurement. Each phase or module is an adaptive test assembled dynamically using the shadow-test approach to CAT, based on an item pool and test blueprint constraints. The current version of **maat** supports an assessment design involving three tests/administrations (e.g. Fall, Winter, and Spring) with two phases within each administration (Phase 1 and Phase 2), so that a student takes six modules in total throughout the year. As a result, the assessment is adaptive in multiple levels:
...
* **Phase**: Each phase involves its own item pool and test specifications (manifested as constraints) to be used in assembly.
* **Module**: Three administrations * two phases give six modules in total.
  * Each student takes six modules across three administrations within the assessment.
...
* **Number of item pools required for the entire assessment**
  * For the full assessment, an item pool is required for each grade in the range. For example, if the range that 4th grade (G4) students can be routed is G3-G6, then all four item pools (G3, G4, G5, and G6) are required for the assessment.
...
* **Routing policy**: Based on a routing policy (described in detail below), students are routed to a grade level after completing each module.
...
* **Item Pool**:
  ...
  * Item parameters from Phase 1 and Phase 2 are on the same metric, and thus can be combined to develop a single ability estimate of the student.
...
* `all`: If a student sees an item, then the item is excluded from shadow tests in all subsquent modules. For example, if a student sees an item in Phase 1 Administration 1, then the item is excluded in Phase 2 Administration 1 and all subsequent phases and administrations. In passage-based assembly, if a student sees at least one item from a passage, then the entire passage is excluded from shadow tests in all following modules.
* `within-test`: If a student sees an item, then the item is excluded from shadow tests in following phases within the current administration. The item is again made available in the subsequent administrations. For example, if a student sees an item in Phase 1 Administration 1, then the item is excluded in Phase 2 Administration 1 but is made available in Phase 1 Administration 2. Similar is done for passage-based assembly.
* `none`: A student can see any item twice (or more) in any phases and administrations. For example, if a student sees an item in Phase 1 Administration 1, the student can see the same item in Phase 2 Administration 1.
...
The **maat** package supports EAP, MLE and MLEF available in the `TestDesign` package for $\theta$ estimation. The estimation method must be specified in `createShadowTestConfig()`.
...
* The **Phase-wise** estimate is obtained by only using the response from the current module. For example, after completing Phase 2 Administration 1, which say had 20 items, the phase-wise estimate is computed based on a 20-item response vector.
...
* As a result of these restrictions, a student can be routed to $G - 1$ at a minimum and $G + 2$ at a maximum. For example, a $G = 4$ student can be potentially routed to grades 3-6. A $G = 4$ student can never be routed to grades 7 and above or below 3 in any module.
...
The first step is to define an assessment structure using the `createAssessmentStructure()` function. In what follows, we specify 3 tests with 2 phases in each test. Route limits are specified to 1 below and 2 above to match the assessment structure diagram shown above. That is, for students in grade $G$, routing is limited to item pools between $G-1$ and $G+2$.
...
* Plot the number of examinees routed to each module. 
* The arguments of the `plotModuleRoutes`: 
  * `examinee_list` is the output object produced by `maat()`.
  * `assessment_structure` is the `assessment_structure` object.

### Route Diagram for the `CI` Transition Policy 

```{r, message = FALSE}
plotModuleRoutes(
  examinee_list        = maat_output_CI, 
  assessment_structure = assessment_structure)
```
...
```{r, message = FALSE}
plotModuleRoutes(
  examinee_list        = maat_output_difficulty, 
  assessment_structure = assessment_structure)
```
...
* The `plot()` function can be used to plot true $\theta$s and estimated $\theta$s across test administrations.
...
* The `plot()` function can also be used to plot interim $\theta$s over modules for a single examinee.
* The arguments of `plot()` for this use case are:
  * `x` is an examinee object.
  * `cut_scores` is a list containing cut scores for each grade.
  
```{r}
examinee <- maat_output_CI[[1]]
